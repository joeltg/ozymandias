...
const index = path.resolve(__dirname, 'web', 'dev.html');
const children = {};

const key = fs.readFileSync(path.resolve(__dirname, 'certs', 'lambda-key.pem'));
const cert = fs.readFileSync(path.resolve(__dirname, 'certs', 'lambda_mit_edu.cer'));
...
// app.get('/:user', (req, res) => res.sendFile(index));
